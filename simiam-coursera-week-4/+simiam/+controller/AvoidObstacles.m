...
% see the LICENSE file included with this software
    ...
    methods
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            %% START CODE BLOCK %%
            %for i=1:5
            %    fprintf('ir_distances_wf_%d (%0.2f , %0.2f)\n',i,ir_distances_wf(i),ir_distances_wf(i+1));
            %end
            
            % Compute the heading vector
            
            n_sensors = length(robot.ir_array);
            sensor_gains = [2 4 6 4 2];
            u_i = zeros(2,5);
           
            u_i = (ir_distances_wf - aux );
            
            for i=1:n_sensors
                fprintf('u_%d (%0.2f , %0.2f)\n',i,u_i(i*2-1),u_i(i*2));
            end
            
            for i=1:n_sensors
                u_i (1:2,i)= u_i (1:2,i)*sensor_gains(i);
            fprintf('u_%d (%0.2f , %0.2f)\n',i,u_i(i*2-1),u_i(i*2));
            end
            
            u_ao = sum(u_i,2);
            fprintf('u_ao (%0.2f , %0.2f)\n',u_ao(1),u_ao(2));
